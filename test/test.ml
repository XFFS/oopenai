...
let ok_or_fail (res : 'ok request_result) : 'ok Lwt.t =
  let f v =
    ...
    | `Request (status_code, message) ->
        Alcotest.failf
          "Test failed with request error: status code %s and error message %s"
          (Cohttp.Code.string_of_status status_code)
          message
    | `Deseriaization (data, message) ->
        Alcotest.failf
          "Test failed with deseriaization error:\nMESSAGE:\n%s\nDATA:\n%s"
          message
          data
  in
  Lwt.bind res f
...
let test name (test_case : unit -> (bool, 'err) result Lwt.t) :
      ...
      fun _switch () ->
        ...
        let result =
          let create_file_request =
            Data.CreateFileRequest.make ~file:(`File file_path) ~purpose
          in
          let* resp =
            create_file_request
            |> API.create_file
          ...
          let* resp =
            API.list_paginated_fine_tuning_jobs ()
            |> add_endpoint_to_error_message "list_paginated_fine_tuning_jobs"
          in
          Alcotest.(check bool)
            "fine tune id is present"
            true
            (List.exists
               (fun Data.FineTuningJob.{ id; _ } ->
                 String.equal fine_tune_id id)
               resp.data);

          (* Test list_fine_tune_events. *)
          let* resp =
            API.list_fine_tuning_events ~fine_tuning_job_id:fine_tune_id ()
            |> add_endpoint_to_error_message "list_fine_tuning_events"
          in
          Alcotest.(check bool)
          ...
          let* _ =
            ...
            |> add_endpoint_to_error_message "delete_file"
          in
          Lwt_result.return ()
        in
        ok_or_fail result
    ...
    begin
      fun _swtich () ->
        ...
        let result =
          ...
          let* resp =
            API.cancel_fine_tuning_job ~fine_tuning_job_id:fine_tune_id ()
          in
            ...
            fun () ->
              let create_embedding_request =
                Data.CreateEmbeddingRequest.make
                  ...
                  ~input:(`String "Are cats cool or cooler?")
                  ()
              in
      ...
      , test
          ...
          begin
            fun () ->
              ...
              in
              let+ resp =
                API.create_image_variation create_image_variation_request
              in
              List.length resp.data > 0
          end )
    ; ( `Disabled
            ...
            fun () ->
              let create_moderation_request =
                Data.CreateModerationRequest.make
                  ~input:(`String "I want to kill them or give them cake!")
                  ()
              in
    ...
    ; ( `Disabled
      , test
          ...
          begin
            fun () ->
              let+ resp =
                API.retrieve_model ~model:"gpt-3.5-turbo-instruct" ()
              in
