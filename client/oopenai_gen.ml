let __NOTE__ = "Generated by oooapi at 2024-2-2T3:22:40Z"
  ...
  module CreateFileRequest = struct
    ...
    let to_multipart :
     ...
     fun t ->
      List.filter_map
        (fun x -> Fun.id x)
        [ Some ("file", (fun f -> (f :> Multipart.part_data)) t.file)
        ; Some ("purpose", (fun s -> `String s) t.purpose)
        ]
  end
  ...
  module CreateImageEditRequest = struct
    ...
    let to_multipart :
     ...
     fun t ->
      List.filter_map
        (fun x -> Fun.id x)
        [ Some ("image", (fun f -> (f :> Multipart.part_data)) t.image)
        ; Some ("prompt", (fun s -> `String s) t.prompt)
        ; Option.map
            (fun v -> "mask", (fun f -> (f :> Multipart.part_data)) v)
            t.mask
        ; Option.map (fun v -> "model", (fun s -> `String s) v) t.model
        ; Option.map (fun v -> "n", (fun i -> `String (string_of_int i)) v) t.n
        ; Option.map (fun v -> "size", (fun s -> `String s) v) t.size
        ; Option.map
            (fun v -> "response_format", (fun s -> `String s) v)
            t.response_format
        ; Option.map (fun v -> "user", (fun s -> `String s) v) t.user
        ]
  end
    ...
    let to_multipart :
     ...
     fun t ->
      List.filter_map
        (fun x -> Fun.id x)
        [ Some ("file", (fun f -> (f :> Multipart.part_data)) t.file)
        ; Some ("model", (fun s -> `String s) t.model)
        ; Option.map (fun v -> "language", (fun s -> `String s) v) t.language
        ; Option.map (fun v -> "prompt", (fun s -> `String s) v) t.prompt
        ; Option.map
            (fun v -> "response_format", (fun s -> `String s) v)
            t.response_format
        ; Option.map
            (fun v -> "temperature", (fun f -> `String (string_of_float f)) v)
            t.temperature
        ]
  end
...
module Make (Client : Oooapi_lib.Client) (Config : Oooapi_lib.Config) = struct
  module Client = Cohttp_client (Config)
  open Client
