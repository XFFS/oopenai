...
module Make (Config : Auth) = struct
  ...
  let add_default_headers hs =
    let org_id_header =
      match Config.org_id with
      | None -> []
      | Some id -> [ "OpenAI-Organization", id ]
    in
    let defaults =
      [ "Authorization", "Bearer " ^ Config.api_key ] @ org_id_header
    in
    Cohttp.Header.add_list hs defaults

  let default_headers = Cohttp.Header.init () |> add_default_headers
